...
func TestTrigLookupTables(t *testing.T) {
	...
	for i := 0; i <= N; i++ {
		p := float64(i) / N
		a := (p*20 - 10) * math.Pi
		cosError := math.Abs(cos(a) - math.Cos(a))
		sinError := math.Abs(sin(a) - math.Sin(a))
		maxCosError = math.Max(maxCosError, cosError)
		maxSinError = math.Max(maxSinError, sinError)
		if cosError > tolerance {
			t.Fatalf("cos(%v) = %v, math.Cos(%v) = %v (%g)", a, cos(a), a, math.Cos(a), cosError)
		}
		if sinError > tolerance {
			t.Fatalf("sin(%v) = %v, math.Sin(%v) = %v (%g)", a, sin(a), a, math.Sin(a), sinError)
		}
	}
